...
public class Persistencia {
    ...
    private static void inicializarAnimales() throws InvalidPropertiesFormatException {
        animales.add(new Carnivoro(5,250,especies.get(0), sectores.get(1), new Pais ("Haiti", "HT")));
        animales.add(new Carnivoro(2,180,especies.get(2), sectores.get(3), new Pais ("Bolivia", "BOL")));
        animales.add(new Herbivoro(3, 1020,especies.get(1), sectores.get(0), 170, new Pais ("Africa", "AFC")));
        animales.add(new Herbivoro(8, 3800,especies.get(3), sectores.get(2), 320, new Pais ("Peru", "PE")));
    }
    ...
    public static void addAnimal(Mamifero animal){
        animales.add(animal);
    }
    
    public static void addAnimal(int edad, double peso, int numEspecie, int numSector, double valorFijo, String pais, String iso) throws InvalidPropertiesFormatException{
        if(valorFijo!=0){
            animales.add(new Herbivoro(edad, peso, especies.get(numEspecie), sectores.get(numSector), valorFijo, new Pais(pais,iso)));
        }else{
            animales.add(new Carnivoro(edad, peso, especies.get(numEspecie), sectores.get(numSector), new Pais(pais,iso)));
        }
    }
